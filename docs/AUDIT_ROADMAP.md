...
| B-3 | **Challenger ağırlık eğitimi:** mevcut `auto_weight_trainer`'ı challenger kanalına yönelt (`backtest_recon.read_challenger()` girdisi; rejim-filtreli) → rejim başına challenger ağırlık seti + quantum'un rejim-bazlı ayrım karnesi (cat 6 cevabı). Champion (canlı) ağırlıkla shadow kıyas. Canlı ağırlığa YAZMAZ | ⬜ B-2 sonrası | 🟡 | F5-2 deseni |
